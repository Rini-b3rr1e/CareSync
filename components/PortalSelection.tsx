...
const PortalCard: React.FC<PortalCardProps> = ({ icon, title, description, buttonText, bgColorClass, hoverBgColorClass, textColorClass }) => (
    <div className="bg-white rounded-2xl shadow-xl p-8 flex flex-col items-center text-center transform hover:-translate-y-2 transition-transform duration-300 w-full">
        ...
        <button className={`${bgColorClass} ${textColorClass} ${hoverBgColorClass} font-bold py-3 px-8 rounded-lg shadow-md hover:shadow-lg transition-all duration-200`}>
            {buttonText}
        </button>
    </div>
);
